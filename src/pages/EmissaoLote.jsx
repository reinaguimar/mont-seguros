...
export default function EmissaoLote() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Seleção de filial + LMI (sempre visível) */}
      <SeletorFilialLmi
        filiais={filiais}
        filialSelecionada={filialSelecionada}
        setFilialSelecionada={setFilialSelecionada}
        rcfvLmiSelecionado={rcfvLmiSelecionado}
        setRcfvLmiSelecionado={setRcfvLmiSelecionado}
      />
      ...
      {total > 0 && (
        <>
          ...
          {/* Confirmação da filial + LMI selecionados */}
          <div className="border-2 border-amber-400 bg-amber-50 rounded-xl p-4 flex items-center gap-3 flex-wrap">
            <AlertCircle className="w-5 h-5 text-amber-500 flex-shrink-0" />
            <p className="text-sm text-amber-900">
              <span className="font-semibold">Filial:</span> {filialSelecionada?.nome || "—"} — Código {filialSelecionada?.codigo_filial || "—"}
              <span className="mx-2 text-amber-300">•</span>
              <span className="font-semibold">LMI RCF-V:</span> R$ {rcfvLmiSelecionado.toLocaleString("pt-BR")}
            </p>
